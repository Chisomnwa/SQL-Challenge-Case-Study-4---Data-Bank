...
-- There were more number of deposits (2671) followed by purchases(1617)and then withdrawals (1580).
...
SELECT AVG(deposit_count) AS avg_depoist_count,
	   AVG(deposit_amount) AS avg_deposit_amount
FROM deposit_summary;
...
/*
3. For each month - how many Data Bank customers make more than 1 deposit and either one purchase
or withdrawal in a single month?

Steps:
* Create a CTE named customer_activity that calculates the number of deposits, purchases and withdrawals 
for each customer for each month using the DATEPART and DATENAME functions to extract the month number 
...
/*
March had the highest number of customers (192) who had made more than 1 deposit and either 1 withdrawal or 1 deposit
while April had the least number of such customers (70).
*/
...
* In the SELECT clause of the CTE,
 - Use the DATEADD function to truncate the txn_date column to the beginning of the month. This is done to group
   the transactions by month, regardlesss of the actual day of the month when the transaction was made.
...
* Use a final query to calculate the closing balance of a customer for a specific month, with the closing balance
  being the sum of all transaction amounts up to and including that month. In the final query:

 - Use the MONTH and DATENAME functions to extract the month id and name from the month_start column of the CTE.

 - Use the SUM function with the OVER clause to calculate the running total of the total_amount column for each 
   customer, partitioned by the customer id and ordered by the month start. This running total gives the closing
   balance of each customer at the end of the month.
*/
